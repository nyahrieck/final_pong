...
from arcade.color import RED, BLUE, WHITE, DARK_GREEN, GRAY
from arcade.key import R, F, UP, DOWN
...
WIDTH = 1000
HEIGHT = 700
SCREEN_TITLE = "One-two Player Ping Pong Game"
GAME_OVER = 5
...
class MenuView(arcade.View):
    ...
    def on_draw(self):
        self.clear()
        arcade.draw_text("Time to Play Ping Pong. ", WIDTH / 2, HEIGHT / 2 + 200,
                         arcade.color.WHITE, font_size=20, anchor_x="center")
        arcade.draw_text("Player one use keys R to move your paddle up and F to move your paddle down.", WIDTH / 2, HEIGHT / 2 + 150,
                         arcade.color.WHITE, font_size=20, anchor_x="center")
        arcade.draw_text("Player two use the up and down arrows. ", WIDTH / 2, HEIGHT / 2 + 100,
                         arcade.color.WHITE, font_size=20, anchor_x="center")
...
class Player(Section):
    ...
    def __init__(self, left: int, bottom: int, width: int, height: int,
        ...
        # the player paddle
        self.paddle: SpriteSolidColor = SpriteSolidColor(30, 100, RED)
...
class Pong(arcade.View):
    ...
    def __init__(self):
        ...
        self.left_player: Player = Player(
            0, 0, PLAYER_SECTION_WIDTH, self.window.height, key_up=R,
            key_down=F, name='Left')
        ...
        # create the ball
        self.ball: SpriteCircle = SpriteCircle(20, WHITE)
    ...
    def end_game(self, winner: Player):
        ...
        if winner.score >= 2:
            self.ball.change_x = random.choice([-3, -2, 8, 10])
            self.ball.change_y = random.choice([-3, -2, 8, 10])
        else:
            self.setup()  # prepare a new game

    def on_draw(self):

        self.clear(DARK_GREEN)  # clear the screen
